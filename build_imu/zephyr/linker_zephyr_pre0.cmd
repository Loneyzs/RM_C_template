 ...
 *(.ARM.exidx* gnu.linkonce.armexidx.*)
 ...
 shared_sw_isr_table :
 {
  KEEP(*(.gnu.linkonce.shared_sw_isr_table*))
 } > FLASH
 initlevel_error :
 {
  KEEP(*(SORT(.z_init_*)))
...
ztest :
{
 ...
 sensor_decoder_api_area : { _sensor_decoder_api_list_start = .; KEEP(*(SORT_BY_NAME(._sensor_decoder_api.static.*))); _sensor_decoder_api_list_end = .;; } > FLASH
 ...
 *(".kernel.*")
 ...
 rtio_pool_area : ALIGN_WITH_INPUT { _rtio_pool_list_start = .; KEEP(*(SORT_BY_NAME(._rtio_pool.static.*))); _rtio_pool_list_end = .;; } > RAM AT > FLASH
